...
const SectionHead = ({ tag, title, sub, isMobile }) => (
  <Reveal style={{ textAlign: "center", marginBottom: 44, maxWidth: 720, marginLeft: "auto", marginRight: "auto" }}>
    ...
    <h2 style={{ fontSize: isMobile ? 28 : 40, fontWeight: 800, color: "#f1f5f9", letterSpacing: -1.2, lineHeight: 1.12 }}>{title}</h2>
  ...
  return (
    <div style={{ background: "#080c18", color: "#e2e8f0", minHeight: "100vh" }}>
      ...
      <section style={{ padding: "0 28px", position: "relative", zIndex: 2, marginTop: -8 }}>
        <Reveal style={{ maxWidth: 1000, margin: "0 auto", display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4, 1fr)", background: "#0d1526", border: "1px solid #1e293b", borderRadius: 18, overflow: "hidden" }}>
          {[{ e: 10000, sep: true, s: "+", l: "MSMEs Assessed" }, { e: 98.2, d: 1, s: "%", l: "ML Accuracy" }, { e: 30, p: "< ", s: "s", l: "Score Generated" }, { e: 11, l: "Loan Products" }].map((x, i) => (
            <div key={i} style={{ padding: "22px 16px", textAlign: "center", borderRight: !isMobile && i < 3 ? "1px solid #1e293b" : "none", borderBottom: isMobile && i < 2 ? "1px solid #1e293b" : "none" }}>
      ...
      <section style={{ padding: isMobile ? "52px 20px" : "72px 28px" }}>
        <Reveal style={{ maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
          ...
          <h2 style={{ fontSize: isMobile ? 28 : 40, fontWeight: 800, color: "#f1f5f9", lineHeight: 1.2, marginBottom: 18, letterSpacing: -1 }}>
          ...
          <p style={{ fontSize: 16.5, color: "#94a3b8", lineHeight: 1.75 }}>
            Most small businesses can't get a loan because they lack formal balance sheets or a CIBIL history. FinHealth AI reads the <strong style={{ color: "#cbd5e1" }}>alternate data they already generate</strong> and produces a bank-grade, explainable credit assessment — for both the business owner and the lending officer.
          </p>
        </Reveal>
      </section>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gap: 16, gridTemplateColumns: isMobile ? "1fr" : "repeat(4, 1fr)", gridTemplateAreas: isMobile ? "none" : `"score score chat stat" "score score chat ntc" "pillar pillar loan shap"` }}>
          <Reveal style={{ gridArea: isMobile ? "auto" : "score" }}>
            <div style={{ ...tile, background: "linear-gradient(160deg, #12213f, #0c1424)", padding: 26 }} onMouseEnter={hIn} onMouseLeave={hOut}>
              ...
              <div style={{ display: "inline-flex", alignItems: "center", gap: 14, background: "#0b1220", border: "1px solid #1e293b", borderRadius: 14, padding: "14px 18px" }}>
          ...
          <Reveal delay={0.05} style={{ gridArea: isMobile ? "auto" : "chat" }}>
            <div style={{ ...tile }} onMouseEnter={hIn} onMouseLeave={hOut}>
              ...
              <div style={{ background: "#111a2e", border: "1px solid #1e293b", borderRadius: 10, padding: 10, fontSize: 11.5, color: "#94a3b8", lineHeight: 1.5 }}>✦ Route UPI collections through your primary account and clear overdue payments…</div>
            </div>
          </Reveal>
      ...
      <section style={{ padding: isMobile ? "20px 20px 52px" : "20px 28px 72px", background: "#0b1120", borderTop: "1px solid #111a2e", borderBottom: "1px solid #111a2e" }}>
      ...
      <section id="how" style={{ padding: isMobile ? "52px 20px" : "72px 28px" }}>
        ...
        <Reveal style={{ maxWidth: 1200, margin: "24px auto 0" }}>
          <div style={{ background: "#0b1120", border: "1px solid #1e293b", borderRadius: 18, padding: "24px" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 14 }}>
              {DATA_SOURCES.map((s) => (
                <div key={s.label} style={{ display: "flex", alignItems: "center", gap: 12, background: "#0f1a30", border: "1px solid #1e293b", borderRadius: 12, padding: "12px 18px", minWidth: 200 }}>
                  <span style={{ fontSize: 22 }}>{s.icon}</span>
                  <div><div style={{ fontSize: 13.5, fontWeight: 600, color: "#e2e8f0" }}>{s.label}</div><div style={{ fontSize: 11.5, color: "#64748b" }}>{s.desc}</div></div>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ═══ NTC spotlight ═══ */}
      <section style={{ padding: isMobile ? "52px 20px" : "72px 28px", background: "#0b1120", borderTop: "1px solid #111a2e" }}>
        <Reveal style={{ maxWidth: 1040, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? 24 : 48, alignItems: "center" }}>
            <div>
              ...
              <h3 style={{ fontSize: isMobile ? 24 : 30, fontWeight: 800, color: "#f1f5f9", marginBottom: 14, letterSpacing: -0.5 }}>Credit for the credit-invisible</h3>
      ...
      <section style={{ padding: isMobile ? "52px 20px" : "72px 28px" }}>
        ...
        <Reveal style={{ maxWidth: 900, margin: "0 auto", background: "#0f1a30", border: "1px solid #1e293b", borderRadius: 18, overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1.2fr 1fr 1fr" : "1.4fr 1fr 1fr" }}>
            <div style={{ padding: "14px 16px", fontSize: 11, color: "#64748b", fontWeight: 700, letterSpacing: 0.5, borderBottom: "1px solid #1e293b" }}></div>
            <div style={{ padding: "14px 16px", fontSize: 12, color: "#94a3b8", fontWeight: 700, borderBottom: "1px solid #1e293b", background: "#0b1220" }}>Traditional</div>
            <div style={{ padding: "14px 16px", fontSize: 12, color: "#818cf8", fontWeight: 800, borderBottom: "1px solid #1e293b", background: "#1a1f3d" }}>FinHealth AI</div>
            {COMPARE.map((row, i) => (
              <React.Fragment key={i}>
                <div style={{ padding: "14px 16px", fontSize: 13, color: "#cbd5e1", fontWeight: 600, borderBottom: i < COMPARE.length - 1 ? "1px solid #1e293b" : "none" }}>{row[0]}</div>
                <div style={{ padding: "14px 16px", fontSize: 12.5, color: "#64748b", borderBottom: i < COMPARE.length - 1 ? "1px solid #1e293b" : "none", background: "#0b1220" }}>{row[1]}</div>
                <div style={{ padding: "14px 16px", fontSize: 12.5, color: "#86efac", fontWeight: 600, borderBottom: i < COMPARE.length - 1 ? "1px solid #1e293b" : "none", background: "#12182f" }}>✓ {row[2]}</div>
              </React.Fragment>
            ))}
          </div>
        </Reveal>
            ...
            <Reveal key={i} delay={i * 0.1}>
              <div style={{ ...tile, display: "flex", flexDirection: "column", justifyContent: "space-between" }} onMouseEnter={hIn} onMouseLeave={hOut}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 40, height: 40, borderRadius: "50%", background: "#1e293b", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>{t.avatar}</div>
                  <div><div style={{ fontSize: 13.5, fontWeight: 700, color: "#f1f5f9" }}>{t.name}</div><div style={{ fontSize: 11.5, color: "#64748b" }}>{t.role}</div></div>
                </div>
              </div>
            </Reveal>
          ))}
      ...
      {/* ═══ FAQ ═══ */}
      <section id="faq" style={{ padding: isMobile ? "52px 20px" : "72px 28px", background: "#0b1120", borderTop: "1px solid #111a2e" }}>
        ...
        <div style={{ maxWidth: 780, margin: "0 auto", display: "flex", flexDirection: "column", gap: 10 }}>
          {FAQS.map((f, i) => (
            <Reveal key={i} delay={i * 0.04}>
              <div onClick={() => setOpenFaq(openFaq === i ? -1 : i)} style={{ background: "#0f1a30", border: `1px solid ${openFaq === i ? "#6366f155" : "#1e293b"}`, borderRadius: 14, padding: "16px 20px", cursor: "pointer", transition: "border-color 0.2s" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                  <div style={{ fontSize: 14.5, fontWeight: 700, color: "#f1f5f9" }}>{f.q}</div>
      ...
      <section id="security" style={{ padding: isMobile ? "52px 20px" : "72px 28px" }}>
        ...
        <Reveal style={{ maxWidth: 820, margin: "0 auto", display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 14 }}>
          {["🔒 RBI AA Framework", "📝 Signed Consent Artifact", "⏱ 90-Day Data Life", "🧾 Purpose-Bound Access", "🔑 JWT-Secured", "🚫 No Data Sold"].map((t) => (
            <span key={t} style={{ fontSize: 13, color: "#cbd5e1", background: "#0f1a30", border: "1px solid #1e293b", padding: "9px 16px", borderRadius: 20 }}>{t}</span>
          ))}
        </Reveal>
      </section>
      ...
      <section style={{ padding: isMobile ? "12px 20px 56px" : "20px 28px 72px" }}>
        <Reveal style={{ maxWidth: 1100, margin: "0 auto", textAlign: "center", background: "linear-gradient(135deg, #1e293b, #1a1240)", border: "1px solid #3b82f644", borderRadius: 24, padding: isMobile ? "40px 24px" : "56px 32px", position: "relative", overflow: "hidden" }}>
