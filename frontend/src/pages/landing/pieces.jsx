    ...
    <header style={{
      ...
      <div style={{ maxWidth: 1240, margin: "0 auto", padding: "0 28px", height: 66, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }} onClick={() => scrollTo("top")}>
          ...
          <div style={{ fontSize: 16, fontWeight: 800, color: "#f1f5f9" }}>FinHealth <span style={{ color: "#818cf8" }}>AI</span></div>
        </div>
        {!isMobile && (
...
export function ScoreRing({ target = 84, size = 100 }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      <div style={{ width: size, height: size, borderRadius: "50%", background: `conic-gradient(#22c55e ${deg}deg, #1e293b ${deg}deg)` }} />
      <div style={{ position: "absolute", inset: 7, width: inner, height: inner, borderRadius: "50%", background: "#0b1220", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <div style={{ fontSize: size * 0.3, fontWeight: 900, color: "#f1f5f9", lineHeight: 1 }}>{Math.round(v)}</div>
...
export function ProductCard({ navigate, float = true }) {
  return (
    <div style={{
      background: "linear-gradient(160deg, #141f38, #0c1424)", border: "1px solid #2a3a5f",
...
export function StatsRow({ isMobile }) {
  return (
    <div style={{
      ...
      background: "#0d1526cc", border: "1px solid #1e293b", borderRadius: 16, overflow: "hidden", backdropFilter: "blur(6px)",
    }}>
      {STATS.map((s, i) => (
...
export function LandingFooter({ navigate }) {
  return (
    <footer style={{ borderTop: "1px solid #1e293b", background: "#080c18", padding: "40px 28px 28px" }}>
      <div style={{ maxWidth: 1240, margin: "0 auto", display: "flex", flexWrap: "wrap", justifyContent: "space-between", gap: 28 }}>
        <div style={{ maxWidth: 300 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 800, color: "#f1f5f9" }}>FinHealth <span style={{ color: "#818cf8" }}>AI</span></div>
          </div>
