  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "32px 16px" }}>
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontSize: 22, fontWeight: 700, color: "var(--c-text)" }}>My Loan Applications</h2>
      ...
      {loading ? (
        ...
        <div style={{ background: "var(--c-surface)", border: "1px solid var(--c-border)", borderRadius: 20, padding: 48, textAlign: "center" }}>
          ...
          <div style={{ color: "var(--c-text)", fontSize: 16, fontWeight: 700, marginBottom: 6 }}>No applications yet</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {apps.map((a) => {
            ...
            return (
              <div key={a.reference} style={{ background: "var(--c-surface)", border: `1px solid ${meta.color}44`, borderRadius: 16, padding: "18px 20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10, marginBottom: 12 }}>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 700, color: "var(--c-text)" }}>{a.product?.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase())}</div>
                    <div style={{ fontSize: 12, color: "#64748b", marginTop: 2 }}>
                      Ref {a.reference} • {a.interest_rate}% p.a. • {a.tenure_months} mo
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 18, fontWeight: 800, color: "var(--c-text)" }}>₹{((a.loan_amount || 0) / 100000).toFixed(1)}L</div>
                    <div style={{
                        ...
                        <React.Fragment key={s}>
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, flex: 1 }}>
                            <div style={{
                              ...
                              background: done ? statusMeta[s].color : "var(--c-bg)",
                              border: `2px solid ${done ? statusMeta[s].color : "var(--c-surface-2)"}`,
                          ...
                          {i < FLOW.length - 1 && (
                            <div style={{ flex: 1, height: 2, background: i < stepIdx ? statusMeta[FLOW[i + 1]].color : "var(--c-surface-2)", marginBottom: 16 }} />
                          )}
                        </React.Fragment>
                      );
                    })}
